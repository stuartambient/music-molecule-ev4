...
const audioPlayerReducer = (state, action) => {
  ...
  switch (action.type) {
    ...
    case 'set-next-track': {
      /*       console.log('set-next-track payload', action.nextTrack); */
      return {
        ...state,
        nextTrack: action.nextTrack
      };
    ...
    case 'tracks-update-like': {
      const { track_id, field, value } = action.payload;
      /*    console.log(track_id, field, value); */
      return {
        ...state,
...
export const AudioPlayerProvider = ({ children }) => {
  ...
  const initialState = {
    ...
    flashDiv: '',
    setup: '',
    mainTheme: /* window.initialMainTheme */ ''
  };
  ...
  const setInitialPage = useCallback(
    (val) => {
      dispatch({
        type: 'set-page',
        home: val === 'home' ? true : false,
        update: val === 'update' ? true : false,
        player: false,
        library: false,
        tagEditor: false
      });
  ...
  useEffect(() => {
    const checkTable = async () => {
      ...
      } else if (isPopulated.length > 0) {
        setInitialPage('home');
        /*   dispatch({
          type: 'database-populated',
