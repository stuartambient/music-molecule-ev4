...
const Stats = () => {
  ...
  const [navRootHighlight, setNavRootHighlight] = useState(null);
  ...
  const handleOpenFolder = (e) => {
    ...
    if (newRoot !== selectedRoot) {
      console.log(`Updating root from ${selectedRoot} to ${newRoot}`);
      setSelectedRoot(newRoot);
      setNavRootHighlight(newRoot);
    }
  };
  ...
  return (
    <>
      {' '}
      {/* <div className={`stats ${state.mainTheme}`}> */}
      <ul className={`stats--nav ${state.mainTheme}`}>
        <li
          className={statReq === 'totalmedia' ? 'stat stat-selected' : 'stat'}
          id="totalmedia"
          onClick={handleStatReq}
        >
          <p>Total media</p>
        </li>
        <li
          className={statReq === 'topArtists' ? 'stat stat-selected' : 'stat'}
          id="topArtists"
          onClick={handleStatReq}
        >
          <p>By Artists</p>
        ...
        <li
          className={statReq === 'genres' ? 'stat stat-selected' : 'stat'}
          id="genres"
          onClick={handleStatReq}
        >
          <p>By Genres</p>
        </li>
        <li className={statReq === 'directories' ? 'stat stat-selected' : 'stat'} id="directories">
          <p className="directories-option" id="directories" onClick={toggleSubmenu}>
            By Directories
          </p>
        </li>
        {isSubmenuOpen && (
            ...
            {directories.map((item) => {
              return (
                <li className="directories" key={item}>
                  ...
                  <p
                    className={
                      navRootHighlight === item ? 'directory-name root-highlight' : 'directory-name'
                    }
                    id={item}
